...
for (y in 1:length(timepoints)) {
  ...
  results <- matrix(0, ncol = length(CellType), nrow = N_clusters+1)
  results <- data.frame(results)
  rownames(results) <- 0:N_clusters
  colnames(results) <- CellType
  
  print(timepoints[y])
  for (Cl in 0:N_clusters) {
    Tot <- length(UMAP_coordinates[UMAP_coordinates$Cluster==Cl,7])
    for (CT in 1:length(CellType)) {
      results[Cl+1,CT] <- length(UMAP_coordinates[UMAP_coordinates$CellType==CellType[CT] & UMAP_coordinates$Cluster==Cl,7])/Tot
    }
    print(paste(Cl,"-",sum(results[Cl,])>0.999))
    
  }
  write.csv(results, file = paste(name_new_dir_0,"/",timepoints[y],"_Cluster&CellTypes_inverse.csv",sep=""))
  
  results <- results %>%
    rownames_to_column() %>%
    gather(colname, value, -rowname)
  
  results$rowname <- as.numeric(results$rowname)
  ...
  heatmap_plot <- ggplot(results,aes(colname, rowname, fill=value)) + 
    ...
    scale_fill_gradient(low="white", high="green", limits = c(0, 1), breaks = seq(0, 1, by = 0.25)) + # setting gradient color as red and white
    ...
    scale_y_continuous(labels = as.character(0:N_clusters), breaks = 0:N_clusters) +
  ...
  jpeg(paste(name_new_dir_0,"/",timepoints[y],"_Cluster&CellTypes_inverse.jpg",sep=""),width=1920, height=1080)
  print(heatmap_plot)
  dev.off()
...
# ___TESTING____________________________________________________________________________
