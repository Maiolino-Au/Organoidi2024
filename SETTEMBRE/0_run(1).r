...
# Seurat
for (file_name in timepoints[6:8]) {
    tic(file_name)
    ...
    sc_data <- PCA.cluster(sc_data, res = res_list[2], n_dim= dim_list)
    ...
    cluster_markers <- cluster.markers(sc_data, output = F)
    ...
    umap_plot <- DimPlot(sc_data, group.by = "celltype_GPT")
    ...
    ggsave(filename = paste0(dir_results, "/", file_name,"_FeaturePlot.png"),
           plot = FeaturePlot(sc_data, c(genes_of_interest, housekeeping_genes[1])),  
           width = 1920*2, height = 1980*2, units = "px")
    ...
    # Save
    # Seurat Obj
    saveRDS(sc_data, file = paste0(dir_results, "/", file_name, "_SeuratObj.rds"))

    # Cluster
    write.csv(sc_data@meta.data, file = paste0(dir_results, "/", file_name, "_SeuratObj_meatadata.csv"))
